...
## 8. Priorities 5-8 extension, v0.6

The [v0.6 report](../reports/behavior-attribution-robustness.en.md) and `scripts/research_priorities.py` add three fixed OLS specifications per outcome, with identical complete-case observations within each outcome. Outcomes use all calendar days for counts, active days for median order size, and entry cohorts for capped holding duration. Year dummies use 2018 as the baseline. Market/capital covariates are from the preceding daily close. None is claimed exogenous. Entry episodes receive equal weight, so the numerous short early episodes dominate the duration sample; this is not a model of an equal-weight average investor-year.

For calendar HAC, daily scores are `s[d] = sum(x[i] * residual[i])` for observations dated `d`. Missing dates have zero scores. The covariance sandwich uses `sum(s[d] s[d]')` plus symmetric lag cross-products weighted `1 - lag/(L+1)` for lags 1 through L, and finite-sample factor `n/(n-k)`. A separate block-wild residual calculation fixes X, assigns ±1 independently per contiguous L-day calendar block anchored on 2018-03-01, and refits through the OLS linear update. L is 7, 14 or 28, with 499 draws and seed `20260923 + L`. Intervals are unadjusted empirical quantiles, not simultaneous tests. Sources S26-S27 provide methodological context; no survival model is fitted. All observed holding endpoints capped at 168 hours are available, including the final open episode.

The BTC attribution is an additive cash-flow-adjusted gain, with gross PNL, signed commissions, funding and change in reference unrealised PNL conserved. USD attribution uses prior wallet/unrealised BTC multiplied by the daily BTC/USD change, plus current-day BTC components multiplied by the current close. External BTC flows use that same close. The decomposition convention allocates interaction terms to current-day converted components. Daily missing valuations propagate, and annual boundary calculations are separately reconciled. This does not turn conditional valuation into observed exchange equity or a valid full-period percentage return.

`scripts/research_sensitivity.py` evaluates the same monthly mark captures on nested 10/60/300-second grids and 30/60/300-second price ages. Strict as-of availability uses both exchange and collection timestamps; incomplete active-price sets remain missing. Same-side adjacent-order bursts use the running maximum execution end within a contiguous same-side run, reset across side/year boundaries. Burst rules never replace real order IDs. The existing five cost-allocation alternatives and three cash timing conventions are retained as separately labelled sensitivity inputs.

Loss definitions use the worst 5% of eligible negative posting days in either BTC or a fraction of `t-2` reference BTC equity. Greedy severity-first declustering requires 62-day separation. Pre-windows end at `t-2`, post-windows begin at `t+1`; matching covariates are known through `t-2`. Matching uses same-year/regime controls, historical standard-deviation scaling, no distance caliper, and published exclusions/distances. Quantile cutoffs, declustering and matching are retrospective; neither event separation nor matching ensures independent shocks or causal identification. Recovery is of cumulative realised ledger PNL, not marked wealth. Missing response values are excluded from denominators rather than counted as non-decreases.

Reproduction, all numerical tables, validation and input hashes are linked in the bilingual report. The separate [v0.6 validation](../results/robustness_research/validation.json) includes an unchanged-source check, conserved realised amounts and annual gains, nested coverage, and event exclusions. Current 2026 market observations are not refreshed by this extension.
