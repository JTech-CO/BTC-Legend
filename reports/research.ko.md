...
포트폴리오 업데이트: [v0.4 전체 복원](portfolio-risk.ko.md)에서 46개 계약의 수량·원가 회계를 확장하고 조건부 현물 기준가격 평가와 스트레스 분석을 추가했습니다. 아래 v0.1 통계는 비교용 기존 결과입니다.

추가 연구: [v0.5 마크가격·장중 노출·입출금 조정 수익률](marks-intraday-returns.ko.md)은 관측된 월초 마크 표본과 시각 민감도를, [v0.5 매매 행동 변화](behavior-changes.ko.md)는 주문 빈도·규모·보유 시간·상품 구성을 분석합니다. 기존 기준 연구를 대체하거나 연속적인 실제 NAV 수익률을 확정하지 않습니다.
